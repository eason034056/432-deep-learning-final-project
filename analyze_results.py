...
Experiment Design:
  ...
  - 3 models: MLP, CNN1D (k=3), PointNet Tiny
  - Total: 6 experiments
        ...
        for model in models:
            ...
            if model_dir.exists():
                ...
                if metrics:
                    ...
                    print(f"⚠ No results found for: {dir_name}")
            else:
...
def generate_summary_report(df: pd.DataFrame, output_dir: Path):
    ...
    with open(report_path, 'w', encoding='utf-8') as f:
        ...
        f.write("Total Experiments: 2 × 3 = 6\n")
        f.write("=" * 100 + "\n\n")
        ...
        for model in df['model'].unique():
            ...
            if len(with_center) > 0 and len(no_center) > 0:
                ...
                if diff > 2:
                    f.write(f"  → Centering significantly improves performance\n")
                elif diff < -2:
                    f.write(f"  → Centering hurts performance\n")
                else:
        ...
        for idx, row in df.iterrows():
            ...
            if row['overfitting_gap'] > 20:
                f.write(f"  → Severe overfitting\n")
            elif row['overfitting_gap'] > 10:
                f.write(f"  → Moderate overfitting\n")
            else:
                f.write(f"  → Good generalization\n")
...
def main():
    ...
    print(f"  📄 {results_dir}/summary_report.txt")
    print(f"  📊 {results_dir}/performance_comparison.png")
    print(f"  📊 {results_dir}/centering_impact.png")
    print(f"  📊 {results_dir}/model_ranking.png")
    print()
    print("View Report:")
